...
export class CamundaController {
    ...
    async completeReview(@Body() reviewDto: ReviewDTO, @Param('taskId') taskId: string){
        try{
            const camundaResponse = await this.camundaService.completeReviewTask(reviewDto.reviewScore, taskId);
